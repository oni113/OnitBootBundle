...
public class BoardService {
    ...
    public Long post(BoardFormDto boardFormDto) {
        Board board = Board.createBoard(
            ...
            authenticationService.getLoginUserInfo().userId()
        );
    ...
    public Long deleteBoard(Long boardId) {
        Board target = Optional.ofNullable(boardRepository.find(boardId))
            .orElseThrow(() -> new CommonBizException(CommonBizExceptionCode.DATA_NOT_FOUND));
        validateUserEditable(target.getWriter().getWriterId());
    ...
    public Long deleteComment(Long boardId, Long commentId) {
        ...
        validateUserEditable(comment.getWriter().getWriterId());
        boardRepository.deleteComment(boardId, commentId);
        return 1L;
    }

    private void validateUserEditable(Long writerId) {
        UserInfoDto loginUser = authenticationService.getLoginUserInfo();
        if (!(writerId.equals(loginUser.userId()) || loginUser.hasAdminRole())) {
